...
async function fetchPosts() {
  ...
  try {
    ...
    if (props.authLogin && props.authApiKey) {
      invokeParams.login = props.authLogin;
      invokeParams.apiKey = props.authApiKey;
    }
    const res = await invoke("fetch_posts", invokeParams);
    // console.log(res);
    ...
    if (isWaterfall.value) {
      images.value = images.value.concat(arr).filter(img => {
        const w = img.width || 1;
...
function handleImageError(event, index) {
  ...
  // 第1次：降级 HTTPS → HTTP
  if (img._loadAttempt === 1 && img.src && img.src.startsWith("https://")) {
...
/**
 * 切换布局：网格→瀑布流时过滤极端比例，瀑布流→网格时重置数据
 */
...
<template>
  <main class="container" id="imagesDev" >
    <div class="section">
          ...
          <div v-for="(img, index) in images" :key="index" class="image-item">
            ...
            <div class="image-container">
              <div v-if="showCheckboxes" class="image-checkbox">
               ...
               <el-image :src="img.src" lazy class="extracted-image" @click="handleImageClick(img, index)" @error="handleImageError($event, index)" fit="cover" show-progress>
        ...
        <MasonryGrid
          ...
          <div  class="image-item" :id="img.md5">
            <div class="image-container">
              ...
              <el-image lazy :src="img.src" class="extracted-image" fit="cover" show-progress @click="handleImageClick(img, index)" @error="handleImageError($event, index)">
                <template #viewer-error="{ activeIndex, src }">
                  <div class="image-slot viewer-error">
                    <el-icon><icon-picture /></el-icon>
                    <span>
      ...
      <div class="pagination-container" v-if="!isWaterfall && total > 0" >
         <el-pagination
